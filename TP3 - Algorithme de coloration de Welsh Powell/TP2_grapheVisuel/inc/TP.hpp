...
public:
	...
	{
		m_graph.ClearEdges();
		CGraph::pNodeSet NodeList = m_graph.GetNodes();
		for (CNode::pNode i : NodeList)
		{
			for (CNode::pNode j : NodeList)
			{
				if (i != j)
				{
					if (!m_graph.CheckNodesAreNeighbors(i, j))
					{
						AddEdge(i, j);
					}
				}
	...
	std::vector<CNode::pNode> GetUnColored()
	{
		std::vector<CNode::pNode> unColoredNodes;
		for (CNode::pNode iNode : m_graph.GetNodes())
		{
			if (iNode->GetColor() == -1)
			{
				unColoredNodes.push_back(iNode);
			}
		}
		return unColoredNodes;
	}

	void ColorizeGraphFull()
	{
		using pNodeVec = std::vector<CNode::pNode>;
		// Assigantion de la couleur -1 (unassigned) à tout les noeuds
		for (CNode::pNode iNode : m_graph.GetNodes())
		{
			iNode->SetColor(-1);
		}
		...
		// Tri des pNodes par degré croissant
		std::sort(unColored.begin(), unColored.end(), ByDegre());

		while (!unColored.empty())
		{
			// Noeud de plus haut degré
			CNode::pNode NodeToColor = unColored.back();
			unColored.pop_back(); // Suppression du noeud
